...
def isTileAligned(tiles, sourceTileId, imageTileGrid, imageAdjList, occupiedTiles, imageTileMap):
    sourceGrid = tiles[sourceTileId]

    for targetTileId, targetTileGrid in tiles.items():
        if (targetTileId == sourceTileId or targetTileId in imageTileGrid):
            continue
        for pos in range(4):
            for sourceOp in ALL_ORIENTATIONS:
                for targetOp in ALL_ORIENTATIONS:
                    sourceGrid = sourceOp(sourceGrid)
                    targetGrid = targetOp(targetTileGrid)

                    if ( isTileBorderAligned( sourceGrid, targetGrid, DIRECTIONS[pos] ) ):
                        print(pos, sourceTileId, targetTileId, DIRECTIONS[pos])

                        targetPos = ""
                        targetCoord = (0,0)
                        if (DIRECTIONS[pos] == "top"):
                            targetPos = "bottom"
                            targetCoord = (0,1)
                        elif (DIRECTIONS[pos] == "right"):
                            targetPos = "left"
                            targetCoord = (1,0)
                        elif (DIRECTIONS[pos] == "left"):
                            targetPos = "right"
                            targetCoord = (-1, 0)
                        elif (DIRECTIONS[pos] == "down"):
                            targetPos = "top"
                            targetCoord = (0,-1)

                        imageTileGrid[sourceTileId] = ImageTile(sourceTileId, sourceGrid, (0,0))
                        imageTileGrid[targetTileId] = ImageTile(targetTileId, targetGrid, targetCoord)
                        imageTileMap[targetCoord] = targetTileId
                        occupiedTiles.append(targetCoord)
                        return True
...
def removeTileBorders(grid, tileDimension):
    # remove cols
    imageRows = []
...
def part1():
    ...
    imageGrid = removeTileBorders(imageGrid, tileDimension)
